    ...
    $AzureFunctionId = az webapp identity show --resource-group $env:AdsOpts_CD_ResourceGroup_Name --name $env:AdsOpts_CD_Services_CoreFunctionApp_Name --query principalId --out tsv
    $WebAppID = az webapp identity show --resource-group  $env:AdsOpts_CD_ResourceGroup_Name --name $env:AdsOpts_CD_Services_WebSite_Name --query principalId --out tsv 
