...
Write-Host "Creating directory to download the SHIR installable."
New-Item -Path $ADFLocalDrive -Name $ADFLocalVMFolder -ItemType Directory -Force                            #'-Force' Ok if directory already exists.

Write-Host "Downloading the SHIR installable at $ADFIRInstallerLocalFileLocation."
Invoke-WebRequest -Uri $ADFIRDownloadURL -OutFile $ADFIRInstallerLocalFileLocation                          #Download SHIR installable.

Write-Host "Downloading the OpenJDK for SHIR at $ADFJDKInstallerLocalFileLocation."
...
Write-Host $MSIInstallArguments
